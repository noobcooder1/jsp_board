...
public class BoardDao {
    ...
    public BoardDto selectOne(int num, boolean incHits) {
            ...
            try (
             ...
             if (rs.next()) {
                ...
                if (incHits) {stmt.executeUpdate("update board set hits=hits+1 where num=" + num);
                 }
             }
        } catch(Exception e) {
            e.printStackTrace();
    ...
    public void insertOne(BoardDto dto) {
             ...
             stmt.executeUpdate(String.format("insert into board " + "(writer, title, content, regtime, hits)" + "values ('%s', '%s', '%s', '%s', 0)", dto.getWriter(), dto.getTitle(), dto.getContent(), getCurrentTime()));
        } catch(Exception e) {
            e.printStackTrace();
        }
     }
    ...
    public void updateOne(BoardDto dto) {
      ...
      try (
            ...
            Statement stmt = conn.createStatement();
             ...
             stmt.executeUpdate(String.format("update board set writer='%s', title='%s', " + "content='%s', regtime='%s' where num=%d", dto.getWriter(), dto.getTitle(), dto.getContent(), getCurrentTime(), dto.getNum()));
        } catch(Exception e) {
            e.printStackTrace();
        }
     }
